...
| 요구사항 인터뷰 | `jin-interview-agent` (sonnet) | 계획 기반 상세 요구사항 확보 |
...
### Step 1.5: 요구사항 인터뷰 (Requirements Interview)

태스크 분석 완료 후, `jin-interview-agent`에게 위임하여 각 태스크의 상세 요구사항을 확보합니다.

**위임 프롬프트 템플릿**:
```
다음 태스크 분해 결과를 바탕으로 상세 요구사항을 인터뷰하세요:
...
## 인터뷰 초점
- 각 태스크의 구체적 구현 요구사항
- 병렬 태스크 간 인터페이스 계약 (공유 타입, API 시그니처 등)
- 우선순위 및 제약 조건
- 엣지케이스 및 예외 처리 방침
...
**승인 게이트** — `AskUserQuestion`으로 인터뷰 결과 승인:
...
**선택지:**
1. ✅ **승인** — 이대로 병렬 실행 진행
2. 📝 **보완** — 추가 요구사항 제시 (직접 입력)
3. ⏭️ **건너뛰기** — 인터뷰 없이 바로 실행
```

- **승인** → Step 2로 진행, 인터뷰 결과를 각 태스크 위임 프롬프트에 포함
- **보완** → 추가 입력 반영 후 재승인
- **건너뛰기** → Step 1 결과만으로 Step 2 진행
...
## 상세 요구사항 (Step 1.5 인터뷰 결과)
[이 태스크에 해당하는 인터뷰 확보 요구사항]

## 인터페이스 계약
[병렬 태스크 간 공유 타입, API 시그니처 등]
...
| `skills/jin-interview/SKILL.md` | 상세 요구사항 인터뷰 스킬 (Step 1.5) |
