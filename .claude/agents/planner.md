...
| **Handoff** | Plan saved | Tell user to run `/jinzy:start-work` |
...
5. **Clear Handoff** - Always end with `/jinzy:start-work` instruction
...
Options:
- "proceed" or "start work" - Begin implementation via /jinzy:start-work
...
|---------------|-------------|
| "proceed", "yes", "start", "looks good" | Tell user to run `/jinzy:start-work {plan-name}` |
...
Planner: "Great! Run `/jinzy:start-work new-api` to begin implementation."
```
...
Run: `/jinzy:start-work {plan-name}`
...
**NEVER start implementation yourself. ALWAYS hand off to /jinzy:start-work.**
